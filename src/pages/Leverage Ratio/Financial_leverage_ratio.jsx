import React, { useState } from "react";
import { useData } from "../../Context/DataContext.jsx";

const Financial_leverage_ratio = () => {

    const { setFinancialLeverageRatioData } = useData();
    const [value1, setvalue1] = useState();
    const [value2, setvalue2] = useState();
    const [result, setresult] = useState();

    const handlecal = () => {
        const total = value1 / value2;
        const rounded = parseFloat(total.toFixed(3));
        setresult(rounded);
        setFinancialLeverageRatioData(rounded);
    };

    return (
        <>
            <div>
                <h2>Financial Leverage Ratio: {result}</h2>
                <div>
                    <lable> Average Total Assets:
                        <input type="number" value={value1} onChange={(e) => setvalue1(Number(e.target.value))} />
                    </lable>
                    <lable> Average Total Equity:
                        <input type="number" value={value2} onChange={(e) => setvalue2(Number(e.target.value))} />
                    </lable>
                    <button onClick={handlecal}>Result</button>
                </div>
            </div>
        </>
    );
};

export default Financial_leverage_ratio;

